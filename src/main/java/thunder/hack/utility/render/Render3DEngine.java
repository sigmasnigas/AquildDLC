    ...
    public static void renderGhosts(int espLength, int factor, float shaking, float amplitude, Entity target) {
        ...
        for (int j = 0; j < 3; j++) {
            for (int i = 0; i <= espLength; i++) {
                ...
                Color baseColor;
                if (ModuleManager.aura.colorOnHit.getValue() && ModuleManager.aura.isHitColorActive()) {
                    baseColor = Color.RED;
                } else {
                    baseColor = HudEditor.getColor((int) (180 * offset));
                }
                int color = Render2DEngine.applyOpacity(baseColor, offset).getRGB();
