...
public abstract class PlaceModule extends Module {
    ...
    protected final Setting<ItemSelectSetting> blocksSelect = new Setting<>("BlocksSelect", new ItemSelectSetting(new ArrayList<>()));
    ...
    protected SearchInvResult getBlockResult() {
        ...
        for (String blockId : blocksSelect.getValue().getItemsById()) {
            Block block = getRegisteredBlock(blockId);
            if (block != null) {
                canUseBlocks.add(block);
            }
        }
