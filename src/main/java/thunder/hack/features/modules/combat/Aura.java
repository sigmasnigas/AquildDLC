...
public class Aura extends Module {
    ...
    public final Setting<Boolean> elytraTargetcustom = new Setting<>("ElytraTarget_players", false);
    public final Setting<Bind> cancel = new Setting<>("Bind", new Bind(-1, false, false), v -> elytraTargetcustom.getValue());
    ...
    public final Setting<Boolean> colorOnHit = new Setting<>("ColorOnHit", true);
    public final Setting<Integer> hitColorTime = new Setting<>("HitColorTime", 500, 100, 2000, v -> colorOnHit.getValue());
    ...
    public void auraLogic() {
        ...
        if (ModuleManager.antiBot.isEnabled() && AntiBot.bots.contains(target)) {
            target = null;
            return;
        }
    ...
    private void updateTarget() {
        ...
        if (target != null && rotationMode.getValue() == Mode.Interact) {
            double distanceToTarget = mc.player.squaredDistanceTo(target.getPos());
            if (distanceToTarget > targetDistance.getValue() * targetDistance.getValue()) {
                target = null;
                return;
            }
        }
    ...
    private void updateHitColor() {
        if (colorOnHit.getValue() && hitColorActive) {
            long currentTime = System.currentTimeMillis();
            if (currentTime - lastHitTime >= hitColorTime.getValue()) {
                hitColorActive = false;
            }
        }
    }

    public Color getAuraESPColor() {
        if (colorOnHit.getValue() && hitColorActive) {
            return Color.RED;
        }
        return ClientSettings.auraESPColor.getValue().getColorObject();
    }

    public boolean isHitColorActive() {
        return colorOnHit.getValue() && hitColorActive;
    }
